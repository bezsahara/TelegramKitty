package org.bezsahara.kittybot.doubles;
...
final public class DoubleTransform {
    ...
    public static void appendTo(double v, JsonByteBuffer jbb)
            throws IOException {
        new DoubleTransform().appendDecimalTo(v, jbb);
    }

    private void appendDecimalTo(double v, JsonByteBuffer jbb)
            throws IOException {
        switch (toDecimal(v)) {
            case NON_SPECIAL:
//                char[] chars = new char[index + 1];
                ...
                jbb.ensureAccess(lim);
                for (int i = 0; i < lim; ++i) {
                    jbb.putUnsafeAccess(bytes[i]);
                }
                return;
            case PLUS_ZERO:
                jbb.write("0.0");
