...
internal object InputPollMediaSerializer : KSerializer<InputPollMedia> {
    private val listSerializer0 = ListSerializer(MessageEntity.serializer())

    override val descriptor: SerialDescriptor = buildClassSerialDescriptor("InputPollMedia") {
        element<String>("type")
    ...
    override fun deserialize(decoder: Decoder): InputPollMedia {
        ...
        decodeLoop@ while (true) {
            when (val index = dec.decodeElementIndex(descriptor)) {
                ...
                15 -> {
                    latitude = dec.decodeDoubleElement(descriptor, 15)
                    nativeMask = nativeMask.setBit(0)
                }
                16 -> {
                    longitude = dec.decodeDoubleElement(descriptor, 16)
                    nativeMask = nativeMask.setBit(1)
                }
                17 -> horizontalAccuracy = dec.decodeNullableSerializableElement(descriptor, 17, Double.serializer(), null)
                ...
                else -> throw SerializationException("Unexpected index $index while deserializing InputPollMedia")
            }
        }
        ...
        return when (type ?: throwMissingField("type")) {
            ...
            else -> throw SerializationException("Serializer wasn't found for InputPollMedia with type $type")
        }
    }

    private inline fun throwMissingField(name: String): Nothing {
        throw SerializationException("Missing required field '$name' while deserializing InputPollMedia")
    }
}
